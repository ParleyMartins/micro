...
unsigned int Raiz_Quadrada(unsigned int S){
	if (S == 0){
		...
		return (Raiz_Quadrada(S-1) + S/Raiz_Quadrada(S-1))/2
	}
}
```
...
```
```C
int potencia_pos(int x, unsigned int N){
	if (N == 0){
		return 1;
	} else if (N == 1){
		return x;
	} else {
		return Potencia(x, N-1)
	}
}

int Potencia(int x, int N){
	if (N < 0){
		return 1/potencia_pos(x, -N);
	} else {
		return potencia_pos(x, N);
	}
}
```
